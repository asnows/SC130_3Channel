...
// Target Devices: 
// Tool Versions: 
// Description: DPC(Bad Point Correction)------坏点校正
// 
// Dependencies: 
// 
// Revision:
...
//   assign data_dpc_out  = dpc_out_reg     ;
   ...
   begin
       {matrix_01,matrix_02,matrix_03} <= {matrix_02,matrix_03,matrix_data01};//将matrix_data01从右往左放入{matrix_01,matrix_02,matrix_03}中
...
/*得到的绝对值中找出最小值min_abs7*/
   ...
   always@(posedge pixel_clk)
   begin
       data_Aver_tmp    <= matrix_02 + matrix_11 + matrix_13 + matrix_22;//e 上下左右的数取平均值（b+d+f+h）/4
   ...
   always@(posedge pixel_clk)
   begin
       matrix_12_delay1 <= matrix_12 ;          //延时1个pixel_clk
       matrix_12_delay2 <= matrix_12_delay1 ;   //延时2个pixel_clk
       matrix_12_delay3 <= matrix_12_delay2 ;   //延时3个pixel_clk
       matrix_12_delay4 <= matrix_12_delay3 ;   //延时4个pixel_clk
       matrix_12_delay5 <= matrix_12_delay4 ;   //延时5个pixel_clk
       matrix_12_delay6 <= matrix_12_delay5 ;   //延时6个pixel_clk       
   end
   
   //get dpc data
   always@(posedge pixel_clk)
   begin
       if(min_abs7 > threshold_value)//10'd40
           begin
...
//       href_out_reg  <= tvalid_delay;
       ...
       if((cols_dpc_en == 1'b1) && (rows_dpc_en == 1'b1) && (dpc_en == 1'b1))//第一行第一列最后一行最后一列的数据不需要dpc处理
           begin
   ...
   begin
       ...
       tvalid_reg  <= {tvalid_reg[5:0],s_axis_tvalid};//左移
       tvalid_delay<= tvalid_reg[5]; //s_axis_tvalid延时6个pixel_clk
       
       tuser_reg   <= {tuser_reg[5:0],s_axis_tuser};
       tuser_delay <= tuser_reg[5]; //s_axis_tuser延时6个pixel_clk
       
       tlast_reg   <= {tlast_reg[5:0],s_axis_tlast};
       tlast_delay <= tlast_reg[5]; //s_axis_tlast延时6个pixel_clk
